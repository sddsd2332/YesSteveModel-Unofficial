...
public class DebugAnimationScreen {
    ...
    public void drawScreen(RenderGameOverlayEvent.Post event) {
        ...
        float lerpBodyRot = rotLerp(partialTick, player.yBodyRotO, player.yBodyRot);
        float lerpHeadRot = rotLerp(partialTick, player.yHeadRotO, player.yHeadRot);
        float netHeadYaw = lerpHeadRot - lerpBodyRot;
        boolean shouldSit = player.isPassenger() && (player.getVehicle() != null && player.getVehicle().shouldRiderSit());

        if (shouldSit && player.getVehicle() instanceof LivingEntity vehicle) {
            lerpBodyRot = rotLerp(partialTick, vehicle.yBodyRotO, vehicle.yBodyRot);
    ...
    public static float rotLerp(float p_14190_, float p_14191_, float p_14192_) {
        return p_14191_ + p_14190_ * MathHelper.wrapDegrees(p_14192_ - p_14191_);
    }
